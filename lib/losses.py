...
class CosineSimilarityLoss(nn.Module):
    ...
    def forward(self, x, y):
        cosine_similarities = self.cosine_similarity(x, y)
...
#             term1 = torch.div(torch.matmul(features[i,:], global_protos[labels[i%batch_size].item()].T), self.temperature)
#             term2 = 0
#             for j in range(10):
#                 simi = torch.div(torch.matmul(features[i,:], global_protos[labels[j].item()].T), self.temperature)
#                 term2 += torch.exp(simi)
