...
def mnist_noniid(args, dataset, num_users, n_list, k_list):
    ...
    #idxs根据标签排序后的索引
    idxs = idxs_labels[0, :]
    #label_begin类别起始列表，包含每个类在idxs_labels中的起始索引
    label_begin = {}
    cnt=0
    ...
    for i in range(num_users):
        ...
        #每个客户端对每个类应该有k个样本
        k = k_list[i]
        #k_len为最大k-shot，每个客户端i的k-shot小于k_len，以次来制造数据集偏移
...
def cifar100_noniid(args, dataset, num_users, n_list, k_list):
    ...
    for i in range(num_users):
        ...
        for each_class in classes:
            ...
            end = label_begin[each_class.item()] + num_imgs
            indices = torch.randperm(end - begin)[:k] + begin
            indices, _ = torch.sort(indices)
            user_data = np.concatenate((user_data, idxs[indices]), axis=0)
        dict_users[i] = user_data
        classes_list.append(classes)
        classes_list_gt.append(classes)
    return dict_users, classes_list, classes_list_gt
